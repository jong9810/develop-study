...
// <section>태그 안에는 헤딩 태그가 들어있다.
// ex) #main-login 같은 작은 박스(연관된 컨텐츠들을 묶을 때 사용)

// button : 버튼을 생성해주는 태그
// button 태그로 생성한 버튼은 마우스를 올려도 손가락 모양으로 변하지 않는다.
// 참고) a 태그로 생성한 것들은 마우스를 올리면 손가락 모양으로 변한다.

// form : input 태그와 button 태그를 넣고 묶어주면 버튼을 누를 때마다 submit 이벤트를 발생시킴.
//

// 다른 웹 페이지로 이동할 때는 주로 a 태그로 만든다(마우스 모양이 손가락으로 바뀜).
// 네이버 검색 버튼은 다른 웹 페이지로 이동하는데 왜 button 태그로 만들었을까?
...
// 컨테이닝 블록의 모든 것(Containing Block)
...
// 쌓임 맥락(Stacking Context) -> z-index 속성과 관련있음
// 화면에 태그들이 어떻게 위치하는지, CSS로 어떻게 위치를 바꿀 수 있는지에 대한 내용
// html 뿌리 엘리먼트
// position 속성이 지정되고(static이 아니고) z-index 속성이 auto 이외의 값을 가지는 엘리먼트들
// z-index 값이 auto가 아닌 다른 값을 가진 flex 요소
// 투명도가 1보다 적게 지정된 엘리먼트들(opacity: 불투명함)
// 쌓임 맥락은 다른 쌓임 맥락 안에 포함될 수 있다.
// 부모 쌓임 맥락들은 자식 쌓임 맥락과는 완전히 독립되어 있다.
// 오로지 자손 쌓임 맥락만이 부모 쌓임 맥락의 영향을 받는다.
// ex) z-index 값이 부모1: 100, 부모2: 1, 부모2자식: 1000 이면,
// 부모2자식은 부모2보다 z-index값이 높은 부모1 위로 올라갈 수 없다.
//
...
// 웹을 다 만든 후에는 validator 사이트나 확장 프로그램을 사용해서 웹 표준에 잘 맞는지
// 점검을 해보는 것이 좋다.
