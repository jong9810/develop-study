...
// 함수는 함수 바깥의 데이터(변수)를 많이 사용하지 않을수록 재사용성이 올라간다(함수 내에서 선언된 변수를 주로 사용하면 좋음).
// 함수 바깥의 데이터를 사용하면 바깥의 데이터에 함수가 의존적이게 되므로 재사용성이 떨어진다.
// 프로그램을 짤 때는 애매한 부분이 없어야 한다. 애매한 부분은 기준을 정해서 확실하게 만들어주어야 함.
// 개발을 할 때는 항상 극단적인 상황에서 에러가 자주 발생하므로, 개발 과정에서 테스트 할 더미 데이터를 만들어두고 테스트 할 때 사용하면 좋다.
...
// 배열.unshift(값) : 배열 맨 첫 번째 자리에 값을 추가할 때 사용(원본 변형 O)

// 배열.shift() : 배열 맨 첫 번째 자리의 값을 제거할 때 사용(원본 변형 O)

// 배열.push(값) : 배열 맨 마지막 자리에 값을 추가할 때 사용(원본 변형 O)

// 배열.pop() : 배열 맨 마지막 자리의 값을 제거할 때 사용(원본 변형 O)

// 배열.splice(지우고 싶은 값의 인덱스, 지우고 싶은 값의 개수, 지워진 자리에 추가할 값)
// : 배열 중간의 값을 한 개 또는 여러 개 제거하거나 추가할 때 사용(원본 변형 O)
...
// 부모 클래스의 메서드와 동일한 메서드를 자손 클래스가 사용하는 경우에는 생략해도 된다.
...
// 부모 클래스는 여러 자식 클래스에게 상속해주는 것이 가능하다.
...
// 호출 스택이 비어있으면 일반적으로는 자바스크립트가 끝나지만, 백그라운드나 태스크 큐에 함수들이 있으면 마저 실행을 해준 후 자바스크립트가 종료된다.
...
// 함수 선언식의 경우에는 함수 선언과 할당이 분리되지 않고, 함수 표현식으로 선언한 경우에는 함수 선언과 할당이 분리된다.
// 즉, 함수 선언식으로 선언한 함수는 함수가 선언되기 전에 호출해도 에러가 발생하지 않음.
// 함수 표현식으로 선언한 함수는 선언되기 전에는 값이 할당되지 않기 때문에 함수 선언 전에 호출하면 에러가 발생한다.

// 함수 표현식 : const say = function(){}; (호이스팅 : 선언과 할당 분리 O)
// 함수 선언식 : function say() {} (호이스팅 : 선언과 할당 분리 X)
// 화살표 함수 : const say = () => {};

// document.querySelector('#table tbody') : id가 'table'인 태그 안에 있는 tbody 태그를 선택.
// 태그를 선택할 때, 한 칸 띄우면 자손 그를 선택할 수 있다.
// 칸을 띄우지 않고 중간에 '>'를 넣어주면 자식 태그를 선택할 수 있다.
...
// fragment없이 table태그에 자식 태그를 append 할 경우 append 하는 자식 태그의 개수만큼 화면을 다시 그리기 때문에 브라우저 속도가 매우 느려질 수 있다.
