...
// - 기본 정규형 : 제 1정규형, 제 2정규형, 제 3정규형, BCNF(보이스/코드 정규형)
// - 고급 정규형 : 제 4정규형, 제 5정규형

// - 원자값(Atomic Value) : 더 이상 분리되지 않는 값을 의미한다. 하나의 레코드에서 각 필드에는 원자값만이 정의될 수 있다.
// - 릴레이션(relation) : 주로 테이블(Table)과 같은 의미로 사용되며, 데이터의 집합을 의미한다. 튜플(tuple)과 속성(attribute)으로 구성되어 있다.
...
// 만일 속성 A가 속성 B의 결정자이면 B가 A에 함수적으로 종속한다고 표현한다. 이때 B는 A의 종속자라고도 표현한다(A -> B).

// - 함수적 종속의 종류
// 1) 완전 함수 종속(Full Functional Dependency)
// 종속자가 기본키에만 종속되며, 기본키가 여러 속성으로 구성되어 있을 경우 종속자가 기본키를 구성하는 모든 속성이 포함된 기본키의 부분집합에 종속된 경우이다.
...
// 릴레이션에서 X, Y, Z라는 3개의 속성이 있을 때 X -> Y, Y -> Z 이란 종속 관계가 있을 경우, X -> Z가 성립될 때 이행적 함수 종속이라고 힌다.
...
// 기본키가 아닌 속성이 기본키에 대한 완전 함수적 종속을 만족, *부*분적 함수 종속을 제거한 정규형
