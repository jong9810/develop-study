...
// 그에 반해, 물리적으로 떨어진 데이터베이스에 네트워크로 연결하여
// 단일 데이터베이스 이미지를 보여주고 분산된 작업 처리를 수행하는 데이터베이스를 분산 데이터 베이스라고 한다.
// 분산 데이터베이스를 사용하는 고객이, 시스템이 분산되어있는지 인식하지 못하면서, 자신만의 데이터베이스를 사용하는 것 처럼 사용할 수 있다.
...
// 5) 장애 투명성
// 만약 데이터베이스가 분산되어 있는 각 지역의 시스템이나 통신망에 이상이 발생하더라도, 데이터의 무결성은 보장된다.
// 6) 병행 투명성
