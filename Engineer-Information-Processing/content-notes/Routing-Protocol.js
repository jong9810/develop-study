...
// 해당 알고리즘에는 최적의 경로를 찾는 방법, 관리자가 직접 테이블 설정하는 정적 라우팅 알고리즘, 정보 변화에 능동적으로 대처하는 동적 라우팅 알고리즘이 있다.
...
// 다익스트라 알고리즘과의 차이점은 매 반복마다 모든 간선을 확인한다는 것이다.
...
// 1) 출발 노드를 설정한다.
// 2) 최단 거리 테이블을 초기화한다.
// 3) 다음의 과정을 V(정점(node)의 개수 - 1)번 반복한다.
//    - 모든 간선 E개를 하나씩 확인한다.
...
// Bellman-Ford 알고리즘의 시간 복잡도는 O(VE)이다.

// - 홉(Hop)
// 출발지 부터 목적지까지 지나치는 장비(라우터)의 수를 의미하고 RIP는 이를 매트릭으로 사용한다.
